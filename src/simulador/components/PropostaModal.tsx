...
export default function PropostaModal({ aoFechar }: { aoFechar: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-start justify-center overflow-y-auto bg-black/70 p-4 sm:items-center">
      <div className="my-8 w-full max-w-3xl rounded-2xl border border-sim-line bg-sim-panel p-3 sm:my-0">
        {/* Barra de ações (não vai para o print) */}
        <div className="mb-3 flex items-center justify-end gap-2">
          <button
            onClick={() => window.print()}
            className="inline-flex items-center gap-2 rounded-lg bg-sim-panel-2 px-3 py-2 text-xs font-semibold ring-1 ring-sim-line transition hover:ring-sim-gold/50"
          >
            <Printer className="h-4 w-4" /> Imprimir / Salvar PDF
          </button>
          <button
            onClick={aoFechar}
            className="inline-flex items-center gap-1 rounded-lg px-2 py-2 text-sim-muted transition hover:text-sim-danger"
        ...
        >
          <div className="min-w-0 flex-1">
            <label
              ...
              className="mb-1 block text-[11px] font-semibold uppercase tracking-wide text-sim-muted"
            >
            ...
            <textarea
              ...
              className="w-full resize-none rounded-lg border border-sim-line bg-sim-surface px-3 py-2 text-sm text-sim-ink outline-none transition placeholder:text-sim-muted/60 focus:border-sim-gold/50"
              {...register("observacao", {
          ...
          <button
            type="submit"
            className="inline-flex shrink-0 items-center gap-1.5 rounded-lg bg-sim-mint px-4 py-2.5 text-sm font-bold text-black transition hover:brightness-110"
          >
