...
import { formatBRL, formatPontos } from "@/simulador/lib/format";
import { VALOR_POR_PONTO } from "@/simulador/config/regras-comerciais";
  ...
  const cards = [
    {
      ...
      color: "text-sim-gold bg-sim-gold/10",
    },
    {
      icon: PiggyBank,
      ...
      color: "text-sim-mint bg-sim-mint/10",
    },
    {
      icon: CalendarClock,
  ...
  return (
    <section className="mb-10">
      <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
        {cards.map((card) => (
          <div key={card.label} className="rounded-2xl border border-sim-line bg-sim-panel p-4">
            <div className="flex items-center gap-2">
              ...
              <p className="text-[13px] font-medium text-sim-muted">{card.label}</p>
            </div>
            <p className="mt-3 text-2xl font-black text-sim-ink">{card.value}</p>
            <p className="mt-1 text-[11px] text-sim-muted/70">{card.hint}</p>
          </div>
        ))}
      </div>
    </section>
